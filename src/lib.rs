...
//             .calls()
//             .filter_map(|callview| {
//                 let txFromAddress = format_hex(&callview.transaction.from);
//                 let txToAddress = format_hex(&callview.transaction.to);
//                 if store.has_at(0, txFromAddress) || store.has_at(0, txToAddress) {
